    ...
    <Wrapper>
      <div className="main-container" onClick={()=> setIsOpen(pre => !pre)}>
        <div className="values-container">
          <span className={`label-str ${isOpen || value?.value ? 'on-top':''}`}>{label}</span>
        ...
        <div className="options-list">
          {
            options?.map((individualOption: availableOptions) => (
              <div className={`individual-item ${individualOption.value === value?.value ? 'selected-option':''}`} key={individualOption?.value} onClick={()=> onOptionClick(individualOption)}>
